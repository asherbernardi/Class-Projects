...
public class GrahamScanner implements Painter, ActionListener {
    ...
    private void reset() {
        ...
        Point lowest = toSort.get(0);
        // set p0 to be a point with minimal y-pos
        for (Point p: toSort)
        	if (p.y < lowest.y)
        		lowest = p;
        toSort.remove(lowest);
        final Point p0 = lowest;
        Comparator<Point> byPolarAngle = new Comparator<Point>() {
			public int compare(Point p1, Point p2) {
				return -((p1.x - p0.x)*(p2.y - p0.y) - (p2.x - p0.x)*(p1.y - p0.y));
			}
        };
        toSort.sort(byPolarAngle);
        stack.push(p0);
        stack.push(toSort.get(0));
        stack.push(toSort.get(1));
        toSort.remove(toSort.get(0));
        toSort.remove(toSort.get(0));
        sortedPoints = toSort.iterator();
    ...
    public void actionPerformed(ActionEvent e) {
        if (sortedPoints.hasNext()) {
        	Point pi = sortedPoints.next();
        	while ((stack.top().x - stack.penult().x)*(pi.y - stack.penult().y) - (pi.x - stack.penult().x)*(stack.top().y - stack.penult().y) < 0) {
        		stack.pop();
        	}
        	stack.push(pi);
        }
