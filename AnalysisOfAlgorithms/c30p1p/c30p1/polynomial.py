...
def polyn_product_dchlr(a, b):
    ...
    if n == 1 :
        ...
        alpha = polyn_product_dchlr(a_low, b_low)
        beta = polyn_product_dchlr(a_high, b_high)
        gamma = polyn_product_dchlr([l + h for l, h in zip(a_low, a_high)], [l + h for l, h in zip(b_low, b_high)])
        delta = [gamma[i] - alpha[i] - beta[i] for i in range(len(alpha))]
        r = []
        for i in range(0, n_half):
            r.append(alpha[i])
        for i in range(n_half, n):
            r.append(alpha[i] + delta[i-n_half])
        for i in range(0, n_half):
            r.append(delta[i + n_half] + beta[i])
        for i in range(n_half, n):
            r.append(beta[i])
        return r
...
def polyn_product_dceor(a, b):
    ...
    if n == 1 :
        ...
        alpha = polyn_product_dchlr(a_even, b_even)
        beta = polyn_product_dchlr(a_odd, b_odd)
        gamma = polyn_product_dchlr([e + o for e, o in zip(a_even, a_odd)], [e + o for e, o in zip(b_even, b_odd)])
        delta = [gamma[i] - alpha[i] - beta[i] for i in range(len(alpha))]
        r = []
        r.append(alpha[0])
        for i in range(0, n-1):
            r.append(delta[i])
            r.append(alpha[i+1] + beta[i])
        r.append(beta[n-1])
        return r
