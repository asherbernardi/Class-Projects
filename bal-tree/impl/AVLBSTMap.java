 ...
 * BST from RecursiveBSTMap. The method AVLVerify.verify is run 
 * on the root of the tree and throws an ImbalanceException if the 
 * tree is not a valid AVL tree. 
 * 
...
public class AVLBSTMap<K extends Comparable<K>, V> extends RecursiveBSTMap<K, V, AVLBSTMap.AVLNode<K, V>> {
    ...
    private class AVLRealNode extends RecursiveBSTMap.RealNode<K, V, AVLNode<K, V>>implements AVLNode<K, V>{
       ...
       protected int balance;
        ...
        public AVLNode<K,V> putFixup() {
            ...
            replace.recompute();
            if (this.balance > 1) {
                AVLRealNode oldLeft = (AVLRealNode) replace.left;
                if (oldLeft.balance < 0) {
                    replace.left = oldLeft.leftRotate();
                }
                replace.recompute();
                return replace.rightRotate();
            }
            else if (this.balance < -1) {
                AVLRealNode oldRight = (AVLRealNode) replace.right;
                if (oldRight.balance > 0) {
                    replace.right = oldRight.rightRotate();
                }
                replace.recompute();
                return replace.leftRotate();
            }
            else {
                return replace;
            }
        ...
        private AVLNode<K,V> leftRotate() {
            AVLRealNode newRoot = (AVLRealNode) this.right;
        ...
        private AVLNode<K,V> rightRotate() {
            AVLRealNode newRoot = (AVLRealNode) this.left;
    ...
    * the number of object. This effectively makes AVLNullNode
