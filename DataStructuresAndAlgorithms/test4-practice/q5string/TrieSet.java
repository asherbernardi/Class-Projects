    ...
    TrieSet(String[] items, int start, int stop, int c) {
        ...
        if (start == stop) {
            if (items[start].length() <= c) {
                ...
                children[items[start].charAt(c) - 'A'] = new TrieSet(items, start, stop, c+1);
                return;
            }
        }
        // Recursion
        int currentChar, lastChar = items[start].charAt(c) - 'A';
        int newStart = start, newEnd;
        for (int i = start; i < stop; i++) {
            currentChar = items[i].charAt(c) - 'A';
            if (currentChar > lastChar || i == stop - 1) {
                newEnd = i;
                children[currentChar] = new TrieSet(items, newStart, newEnd, c+1);
                newStart = i;
