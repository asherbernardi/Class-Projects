...
public abstract class Heap<E> {
    ...
    protected void sinkKeyAt(int i) {
        assert isHeapBut(i);
        // Since a heap has the property of almost complete, we can assume that
        // if the left child is null, the right will also be, but not vice-versa
        // Base Case: i is a leaf. Nothing to be done.
        if (left(i) >= heapSize || internal[left(i)] == null) {
            return;
        }
        // Case: i has one child.
        if (right(i) >= heapSize || internal[right(i)] == null) {
            // swap if child greater than i.
            if (compy.compare(internal[left(i)], internal[i]) > 0) {
                swap(left(i), i);
                sinkKeyAt(left(i));
                return;
            }
        ...
        // Case: i has two children
        // Get the index of whichever child is greater.
        int biggestChild;
        if (compy.compare(internal[left(i)], internal[right(i)]) > 0)
            biggestChild = left(i);
        else
            biggestChild = right(i);
        // swap the value at i with the bigger child if necessary.
        if (compy.compare(internal[biggestChild], internal[i]) > 0) {
            swap(biggestChild,i);
            sinkKeyAt(biggestChild);
            return;
        }
        else return;
    }
