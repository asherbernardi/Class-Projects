...
public class BellmanFordSSSP implements SSSP {
    ...
    public Set<WeightedEdge> sssp(WeightedGraph g, int source) {
        ...
        for (int i = 0; i < g.numVertices() - 1; i++) {
            for (WeightedEdge v : g.edges()) {
                if (distanceBounds[v.second] > distanceBounds[v.first] + v.weight) {
                    distanceBounds[v.second] = distanceBounds[v.first] + v.weight;
                    parents[v.second] = v.first;
                }
            }
        }
