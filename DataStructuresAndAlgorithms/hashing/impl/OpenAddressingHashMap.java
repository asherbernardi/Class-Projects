...
public class OpenAddressingHashMap<K, V> implements Map<K, V> {
    ...
    public V get(K key) {
        int pos = find(key);
        return (pos<0? null: table[pos].value);
    }
    ...
    public void put(K key, V val) {
        ...
        Iterator<Integer> probe = prober.probe(key);
        int i;
        // search through the table, keeping track of the first deleted
        boolean delFound = false;
        int firstDel = -1;
        do {
            assert probe.hasNext();
            i = probe.next();
            if (table[i] == deleted && !delFound) {
                firstDel = i;
                delFound = true;
            }
        } while(table[i] != null && ! key.equals(table[i].key));
        // If the key was not found, either fill the first deleted space
        // with the key, or fill the next empty space with a key.
        if (table[i] == null) {
            if (delFound) {
                table[firstDel] = new Pair<>(key, val);
                deleteds--;
                numPairs++;
            }
            else {
        ...
        // If the key was found, update the key's value.
        else if (key.equals(table[i].key)) {
            table[i].value = val;
        }
        // rehash if the proportion of elements in the table exceeds the load factor
        if ((double)(numPairs+deleteds)/(double)(table.length) > loadFactor)
            rehash();
    }
